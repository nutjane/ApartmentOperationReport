...
public class ReportUsedElectricity{
		...
		try {
			...
			report()
				 ...
				 .title(//shows report title
			  	cmp.horizontalList()
			  		.add(
			  			...
			  			cmp.text("Electricity Usage").setStyle(titleStyle).setHorizontalAlignment(HorizontalAlignment.RIGHT))
			  		.newRow()
			  ...
			  .pageFooter(cmp.pageXofY().setStyle(boldCenteredStyle), //shows number of page at page footer
				...
				.summary(//show table
					cht.timeSeriesChart()
						.setTitle("Electricity Usage")
						.setTitleFont(boldFont)
						.setTimePeriod(orderDateColumn)
						.setTimePeriodType(TimePeriod.MONTH)
						.series(cht.serie(unitColumn))
						.setTimeAxisFormat(cht.axisFormat().setLabel("Month/Year")))
		...
		for(int i=0;i<count;i++){
			for(int j=0;j<invoiceData.invoiceCollection.size();j++){
				...
				if(invoiceData.invoiceCollection.get(j).getMonth() == month && 
					...
					usage += invoiceData.invoiceCollection.get(j).getElectricUsed();
				}								
			}
...
//		new ReportUsedElectricity(1,1,1,1);
	}
}