...
public class ReportInvoiceUnpaid { 
	...
	private void build() {	
		...
		StyleBuilder columnTitleStyle  = stl.style(boldCenteredStyle)
				...
				.setBackgroundColor(new Color(255, 210, 210)); //light-red
		...
		TextColumnBuilder<BigDecimal>  total  = col.column("TOTAL",  "total", type.bigDecimalType()).setHorizontalAlignment(HorizontalAlignment.CENTER).setStyle(boldStyle);
			  ...
			  .title(//shows report title
			  	cmp.horizontalList()
			  		.add(
			  			...
			  			cmp.text("Unpaid invoices").setStyle(titleStyle).setHorizontalAlignment(HorizontalAlignment.RIGHT))
			  		.newRow()
	...
	private JRDataSource createDataSource() {
		...
		for(int i=0;i<invoiceData.invoiceCollection.size();i++){
			...
			if(invoiceData.invoiceCollection.get(i).getDatePaid().equals("no")){
			//add data
